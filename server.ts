...
async function startServer() {
  ...
  app.get("/api/search/youtube", async (req, res) => {
    ...
    if (!API_KEY || API_KEY === "YOUR_YOUTUBE_API_KEY") {
      ...
      return res.status(401).json({ error: "YouTube API key is missing or invalid. Please set VITE_YOUTUBE_API_KEY in Secrets." });
    }
  ...
  app.post("/api/search/gemini", async (req, res) => {
    ...
    if (!API_KEY || API_KEY === "YOUR_GEMINI_API_KEY") {
      return res.status(401).json({ error: "Gemini API key is missing or invalid. Please set GEMINI_API_KEY in Secrets." });
    }
